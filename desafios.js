...
function highestCount(numbers) {
	...
	for (let maiorA of numbers) {
		if (maiorA > maiorN){
			maiorN = maiorA;
		}  
	}
	let cont = 0;
	for (const quantasVezes of numbers) {
		if (quantasVezes == maiorN){
			cont += 1;
		}
	}
	return cont;
...
function generatePhoneNumber(numberTel) {
	...
	if (numberTel.length > 11 || numberTel.length < 11){
		return 'Array com tamanho incorreto.';
	}else {
		for (let indice = 0; indice < numberTel.length; indice+=1) {
			let contarNumero = numberTel[indice];
			let cont = 0;
			for(let i of numberTel){
				if (contarNumero === i){
					cont += 1;
					if (cont >= 3 || contarNumero < 0 || contarNumero > 9) {
	...
	let tel = `(${numberTel[0]}${numberTel[1]}) ${numberTel[2]}${numberTel[3]}${numberTel[4]}${numberTel[5]}${numberTel[6]}-${numberTel[7]}${numberTel[8]}${numberTel[9]}${numberTel[10]}`;
	return tel;
}
console.log(generatePhoneNumber([1, 2, 3, 4, 5, 7, 7, 8, 5, 4, 1]));
// O que será verificado:
